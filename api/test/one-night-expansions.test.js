...
test('a shielded Role Retriever is offered a valid confirmation instead of impossible swaps', () => { const ctx = fixture('roleRetriever'); ctx.room.shields.push(ctx.seat.id); const before = JSON.stringify(ctx.room.cards); const d = expansion.buildAction(ctx); assert.equal(d.min, 0); assert.equal(d.max, 0); assert.equal(d.targets.length, 0); doAction(ctx); assert.equal(ctx.done, true); assert.equal(JSON.stringify(ctx.room.cards), before); });
test('Rascal self-swap variant acknowledges its own shield without offering illegal targets', () => { const ctx = fixture('rascal'); ctx.room.expansion.rascalVariant = 'swapSelf'; ctx.room.shields.push(ctx.seat.id); const before = JSON.stringify(ctx.room.cards); assert.equal(expansion.buildAction(ctx).targets.length, 0); doAction(ctx); assert.equal(ctx.done, true); assert.equal(JSON.stringify(ctx.room.cards), before); });
