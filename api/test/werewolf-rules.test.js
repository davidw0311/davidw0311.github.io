...
test('Dead Sheriff resolves badge before voting; cannot change ballot weight during voting',()=>{const r=setup();r.sheriffSeatId=id(r,6);nightAct(r,0,{targetId:id(r,6)});nightAct(r,1,{targetId:id(r,6)});dawn(r);assert.equal(r.phase.step,'badge');assert.throws(()=>send(r,r.hostId,'startVoting'),{code:'BADGE_PENDING'});send(r,'actor6','passBadge',{targetId:id(r,0)});assert.deepEqual(r.phase.publicCues,['badge-passed','seat-1']);send(r,r.hostId,'nightNarrationDone');send(r,r.hostId,'startVoting');assert.throws(()=>send(r,'actor6','passBadge',{targetId:id(r,1)}),{code:'NO_ABILITY'});});
test('Host can hard skip a disconnected dead Sheriff by destroying the badge',()=>{const r=setup();r.sheriffSeatId=id(r,6);nightAct(r,0,{targetId:id(r,6)});nightAct(r,1,{targetId:id(r,6)});dawn(r);send(r,'actor6','leave');send(r,r.hostId,'hardSkip');assert.equal(r.sheriffSeatId,null);assert.deepEqual(r.phase.publicCues,['badge-destroyed']);send(r,r.hostId,'nightNarrationDone');assert.equal(r.phase.step,'discussion');});
...
test('dawn announces selected seat numbers, then nobody silenced after elder dies',()=>{
 ...
 assert.deepEqual(dawnCues(r),['night-deaths','seat-3','last-words-3','silenced-today','seat-6']);
