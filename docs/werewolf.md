...
5. Night turns advance automatically: Brian calls a role, music plays while its players submit or skip, and Brian asks them to close their eyes before the next call. The host controls daytime discussion, voting, Sheriff elections, and when to start the next night. Daytime timers are optional.
6. Brian narration and night music are selected by default; starting the game unlocks sound on the host device. They can be muted independently, and preferences are saved locally. After refreshing or changing hosts, tap **Enable moderator sound** if the browser needs a new gesture. Players choose English or Simplified Chinese independently. Only the host device narrates, avoiding multiple speakers in one room. Text announcements are always available.

The release includes 62 pre-recorded English/Chinese clips made with Azure `en-US-BrianMultilingualNeural`, including individual first-night role calls and separate Sheriff and exile voting prompts. `scripts/generate-werewolf-audio.mjs` regenerates them with authenticated Azure CLI or secure speech environment variables; the manifest records exact text, voice, hashes, and durations. `scripts/generate-werewolf-ambience.py` reproduces the original 32-second seamless night loop using only Python's standard library. During event-driven nights, ambience loops only during the action stage and fades out before the closing announcement. Phase changes cancel obsolete speech.
...
Every new night uses three server-owned stages for each scheduled role: opening announcement, action, and closing announcement. Narration completion is acknowledged automatically by the host device after every clip finishes. No normal host “next step” click is needed. Silent play acknowledges text announcements automatically; bounded server fallback timers prevent unavailable audio from stalling the room.

During the action stage, every eligible living player must explicitly submit or skip, including each participating wolf. Living roles are not skipped when a countdown expires. First-night roles (Cupid, Wild Child, Wolf Hound, Thief, and Mechanical Wolf) receive individual calls; their setup choices resolve together before the wolves act.
...
Pause and disconnect handling freeze stage timing. A replacement continues the same seat’s action. If a turn cannot be recovered, the host can pause and explicitly confirm **Skip a blocked night turn**; missing actions count as skipped and the closing announcement still plays. This is an emergency recovery control, not the normal night flow. Games already in an old night finish that legacy night normally and switch at the next night.
...
- Automatic moderation pauses when the host, any living seat, or an eliminated player with a pending shot has been absent for 35 seconds. The host explicitly resumes when the table is ready. Seat replacement, pause/resume, and the paused-only emergency skip remain available to handle absences.
