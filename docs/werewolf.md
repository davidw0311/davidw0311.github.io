...
5. Night turns advance automatically: Brian calls a role, music plays while its players submit or skip, and Brian asks them to close their eyes before the next call. The action window ends early when everyone submits, or at the configured maximum (45 seconds by default); missing actions are skipped even if players are offline. The host controls daytime discussion, voting, Sheriff elections, and when to start the next night. Daytime timers are optional.
...
The release includes 62 pre-recorded English/Chinese clips made with Azure `en-US-BrianMultilingualNeural`, including individual first-night role calls and separate Sheriff and exile voting prompts. `scripts/generate-werewolf-audio.mjs` regenerates them with authenticated Azure CLI or secure speech environment variables; the manifest records exact text, voice, hashes, and durations. `scripts/generate-werewolf-ambience.py` reproduces the original 32-second seamless night loop using only Python's standard library. Persistent HTML media elements play narration and ambience. A direct tap unlocks both elements before loading asynchronous clips, and source reuse helps mobile autoplay compatibility. During event-driven nights, ambience loops only during the action stage and stops before the closing announcement. Test sound plays a public Brian cue without acknowledging a game stage. Phase changes cancel obsolete speech.
...
During the action stage, eligible living players can submit or explicitly skip, including each participating wolf. All submissions finish the turn early. Otherwise the private action deadline, based on **Maximum night action (sec)** (45 seconds by default), closes the turn with missing choices skipped/defaulted. Submitted actions remain saved. Changing this setting applies to the next action stage, without resetting the current private window. Hunter/Wolf King reactions also finish at their deadline even if optional daytime timers are off. First-night roles (Cupid, Wild Child, Wolf Hound, Thief, and Mechanical Wolf) receive individual calls; their setup choices resolve together before the wolves act.
...
Only an explicit host pause freezes stage timing. Disconnects do not pause the room. A replacement continues the same seat’s action if its window is still open; closed turns cannot be replayed. If a turn cannot be recovered, the host can pause and explicitly confirm **Skip a blocked night turn**; missing actions count as skipped and the closing announcement still plays. This is an emergency recovery control, not the normal night flow. Existing rooms paused automatically because of a disconnection resume on their next synchronization; intentional host pauses stay paused. Existing indefinite action turns receive one persisted deadline on their next synchronization. Games in a legacy timed night switch to the staged flow at the next night.
...
- Offline players and an offline host do not pause automatic progression. Missing night actions and pending shots expire normally. The host may still pause deliberately to wait for someone; seat replacement, pause/resume, and the paused-only emergency skip remain available.
...
Polling is normally 3 seconds in the foreground and 12 seconds in background tabs, with backoff on failures and immediate synchronization after connectivity/visibility changes. During an unpaused night the host polls every second; other players poll every 2 seconds in the foreground or 3 seconds in the background. Heartbeats persist at most once per actor per ten seconds. Server time owns phase deadlines. Automatic transitions are evaluated on requests. Keep at least one room screen connected for live progression; everyone else may go offline. If every device closes the page, state stays saved and overdue stages continue on the next connection, one stage per request rather than silently simulating an entire unattended game.
