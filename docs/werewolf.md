...
5. Night turns advance automatically after players submit: Brian calls each role, music plays during its action, and Brian asks players to close their eyes. Living actions never expire. The host can confirm **Skip current step** at night or during the day without seeing pending players or identities.
...
During the action stage, eligible living players submit or explicitly skip, including each participating wolf. All submissions finish the turn. Otherwise it waits indefinitely, including when players are offline. Hunter/Wolf King reactions also wait for the player or an explicit host skip. First-night setup choices resolve together before wolves act; hard-skipped choices use the documented defaults.
