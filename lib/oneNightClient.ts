...
import backend from "@/public/assets/one-night/backend.json";

import { useCallback, useEffect, useRef, useState } from "react";

export type Language = "en" | "zh";
export type Localized = { en: string; zh: string };
export type Role = { id: string; name: Localized; team: string; description: Localized; expansion?: string; order?: number; maxCount?: number; icon?: string };
export type Preset = { id: string; name: Localized; description: Localized; roles: string[]; roleDeck?: string[]; players?: number };
export type Catalogue = { roles: Role[]; presets: Preset[]; expansions?: { id: string; name: Localized; description?: Localized }[] };
export type Seat = { id: string; name: string; number: number; photo?: string | null; ready: boolean; connected: boolean; occupied: boolean; host?: boolean; isHost?: boolean; roleId?: string; originalRoleId?: string; finalRoleId?: string; mark?: string; shielded?: boolean; hasArtifact?: boolean; revealedRoleId?: string };
...
const endpoint = process.env.NEXT_PUBLIC_ONE_NIGHT_API_URL || backend.url;
const publishableKey = process.env.NEXT_PUBLIC_ONE_NIGHT_PUBLISHABLE_KEY || backend.publishableKey;
...
const sessionKey = "nightfall.one-night.session.v1";
const pendingKey = "nightfall.one-night.pending.v1";
...
export function useOneNightRoom() {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      try {
        ...
        apply(reply.view); failures = 0; nightPolling = reply.view.phase.kind === "night"; hostPolling = reply.view.isHost; setConnectivity("online");
  ...
  const acknowledgeNightNarration = useCallback(async (phaseId: string): Promise<boolean> => {
    ...
    try {
      const reply = await reliableRequest({ op: "command", code: current.code, token: current.token, requestId: `narration-${phaseId}`, command: { type: "narrationDone", expectedPhaseId: phaseId } });
