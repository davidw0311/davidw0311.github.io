...
export type OneNightAudioPhase = {
  id: string;
  kind: string;
  step?: string | null;
  paused?: boolean;
...
export class OneNightAudio {
  ...
  private report: (status: OneNightAudioStatus, message?: string) => void;
  private onNightNarrationDone?: (phaseId: string) => void;

  constructor(report: (status: OneNightAudioStatus, message?: string) => void, onNightNarrationDone?: (phaseId: string) => void) {
    this.report = report;
  ...
  private ensureMedia() {
    ...
    // Kokoro stays on the persistent HTML media playback route.
  ...
  /** Audible Kokoro test, called directly in a gesture. The test never ACKs a phase. */
  ...
  updatePhase(phase: Phase) {
    const key = JSON.stringify([phase.id, phase.kind, Boolean(phase.paused), phase.nightStage ?? "", phase.cueIds ?? []]);
    this.phase = phase;
    if (key === this.phaseKey) { this.syncMusic(); return; }
    this.phaseKey = key;
    this.cues = [...(phase.cueIds || [])];
    this.stopNarration();
    this.replay();
    this.syncMusic();
  }
