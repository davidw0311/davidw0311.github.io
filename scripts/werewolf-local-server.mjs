...
const {WerewolfService}=require('../api/src/werewolf/service.js');
const {makeHandler}=require('../api/src/werewolf/http.js');
class MemoryStore {
...
const port=Number(process.env.WEREWOLF_PORT||4502);
const handler=makeHandler(new WerewolfService(new MemoryStore()),['http://localhost:3010','http://127.0.0.1:3010','http://localhost:3000']);
...
}).listen(port,'127.0.0.1',()=>console.log(`Classic Werewolf development API: http://localhost:${port}`));
