"""Generate offline bilingual One Night recordings, resumable by text/voice hash."""
import hashlib
import json
import os
...
folder = ROOT / 'public/assets/one-night/audio'
...
for language, (repo, voice, code) in settings.items():
    ...
    for cue, translations in texts.items():
        ...
        manifest['clips'][key] = {'src': '/assets/one-night/audio/'+language+'/'+cue+'.mp3', 'text': text, 'duration': duration, 'textHash': fingerprint, 'provider':'Kokoro', 'voice':voice, 'model':repo, 'speed':1.0}
