"""Generate offline bilingual shared Nightfall victory recordings, resumable by text/voice hash."""
import hashlib
import json
import os
...
folder = ROOT / 'public/assets/nightfall/audio'
for lang in ['en','zh']: (folder / lang).mkdir(parents=True, exist_ok=True)
manifest_path = folder / 'manifest.json'
manifest = json.loads(manifest_path.read_text())
texts = json.loads((folder / 'text.json').read_text())
...
for language, (repo, voice, code) in settings.items():
    ...
    for cue, translations in texts.items():
        ...
        manifest['clips'][key] = {'src': '/assets/nightfall/audio/'+language+'/'+cue+'.mp3', 'text': text, 'duration': duration, 'textHash': fingerprint, 'provider':'Kokoro', 'voice':voice, 'model':repo, 'speed':1.0}
        manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2)+'\n')
        print(f'{key}: {duration:.2f}s ({voice})', flush=True)
manifest['provider'] = 'Kokoro'
manifest['voices'] = {'en':'am_michael','zh':'zm_010'}
manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2)+'\n')
