...
for (const story of stories) {
 ...
 for (const [sectionIndex,section] of sections.entries()) {
  for (const [partIndex,part] of section.parts.entries()) {
   const seeress = story.slug === 'voluspa' && section.id.startsWith('quote-');
   const clipVoice = seeress ? seeressVoice : voice;
   const clipFolder = seeress ? folder.replace('/v1/', '/v2/') : folder;
   await mkdir(path.join('public',clipFolder), {recursive:true});
   const src = `${clipFolder}/${sectionIndex}-${partIndex}.mp3`;
   ...
   if (!(duration > 0)) {
    ...
    const words = seeress ? `<prosody rate="-8%" pitch="-8%">${escape(part.text)}</prosody>` : escape(part.text);
    const ssml = `<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="en-US"><voice name="${clipVoice}"><lang xml:lang="${part.lang === 'non' ? 'is-IS' : 'en-US'}"><p>${words}</p></lang></voice></speak>`;
   ...
   clips.push({section:sectionIndex,part:partIndex,lang:part.lang,src,duration,...(story.slug === 'voluspa' ? {voice:clipVoice,speaker:seeress ? 'Seeress' : 'Narrator'} : {})});
  }
