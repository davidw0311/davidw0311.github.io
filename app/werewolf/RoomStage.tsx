...
import { classicVictory, victoryHeading, personalVictory } from "@/lib/nightfallVictory";
...
export function stageText(view: GameView, lang: Language) {
  ...
  if (phase.kind === "day" && phase.step === "badge") return t("Waiting for Sheriff: pass or destroy the badge", "等待警长移交或撕毁警徽");
  ...
  if (phase.kind === "announcement") return phase.step === "badgeResult" ? t("Announcing the badge decision", "公布警徽处理结果") : phase.step === "dayDeaths" ? t("Announcing eliminated players", "公布出局玩家") : phase.step === "sheriffResult" ? t("Announcing the Sheriff result", "公布警长竞选结果") : t("Announcing last night’s result", "公布昨夜结果");
...
export function StageBanner({ view, lang, onCard, onProfile }: { view: GameView; lang: Language; onCard: () => void; onProfile: () => void }) {
  ...
  const victory = classicVictory(view);
  const dead = ownSeat && !ownSeat.alive && view.status === "playing";
...
export function ElectionPanel({ view, lang, disabled, send }: { view: GameView; lang: Language; disabled: boolean; send: (command: Command) => Promise<boolean> }) {
  ...
  return <section className={styles.electionPanel}><h2>{t("Sheriff election", "警长竞选")}</h2><p>{election.round === 2 ? t("Only tied candidates cannot vote in this runoff. All other eligible players may vote, including former candidates and withdrawals.", "复投仅平票候选人不能投票；其他有投票权的玩家均可投票，包括此前上警或退水者。") : t("Last night’s results are hidden until the election ends. Candidates, including anyone who withdraws, cannot vote.", "竞选结束后公布昨夜结果。上警玩家（包括退水者）不能投票。")}</p>
    ...
    {election.round === 2 && <p>{t("Sheriff runoff: tied candidates speak again. A second tie leaves no Sheriff.", "警长复投：平票候选人再次发言；再次平票则无警长。")}</p>}
    ...
    {candidate && election.round !== 2 && view.phase.kind === "sheriff" && <button className={styles.secondaryButton} disabled={disabled} onClick={() => void send({type:"sheriffWithdraw"})}>{t("Withdraw candidacy", "退水")}</button>}
    {view.phase.kind === "sheriff" && election.round !== 2 && election.withdrawnIds.includes(me.seatId) && <button className={styles.secondaryButton} disabled={disabled} onClick={() => void send({type:"sheriffRejoin"})}>{t("Rejoin the election", "重新上警")}</button>}
