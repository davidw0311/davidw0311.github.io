...
export function CircleSeats({ view, lang, now, onProfile }: { view: GameView; lang: Language; now: number; onProfile: () => void }) {
  ...
  return <div className={styles.circleScroll} aria-label={t("Players seated clockwise around the table", "玩家按顺时针入座")}><div className={`${styles.circleTable} ${large ? styles.ovalTable : ""}`} style={large ? { "--seat-rows": rows } as CSSProperties : undefined}>
    ...
    {view.seats.map((seat, index) => {
      ...
      const status = dead ? t("Eliminated", "已出局") : view.phase.kind === "ready" ? seat.ready ? t("Ready", "已准备") : t("Not ready", "未准备") : voting ? view.pendingVoterIds?.includes(seat.id) ? t("To vote", "未投票") : t("Voted", "已投票") : seat.id === view.speakerSeatId ? t("Speaking", "发言中") : seat.silenced ? t("Silenced", "禁言中") : seat.isSheriff ? t("Sheriff", "警长") : mine ? t("You", "你") : !seat.occupied ? t("Reserved", "预留") : !seat.connected ? t("Offline", "离线") : "";
      return <div key={seat.id} style={position} className={`${styles.circleSeat} ${styles.seat} ${dead ? styles.deadSeat : ""} ${mine ? styles.mySeat : ""} ${seat.id === view.speakerSeatId ? styles.speakingSeat : ""} ${!seat.connected ? styles.offlineSeat : ""}`}>
        <div className={styles.circleAvatarWrap}><button className={styles.avatar} disabled={!mine} onClick={onProfile} aria-label={t(`Seat ${index + 1}: ${seat.name}${mine ? ", change profile photo" : ""}`, `${index + 1}号：${seat.name}${mine ? "，更换头像" : ""}`)}><Portrait seat={seat} />{dead && <span className={styles.deadCross} aria-hidden="true">×</span>}<span className={`${styles.presence} ${seat.connected && seat.occupied ? styles.present : ""}`} title={seat.connected ? t("Connected", "在线") : t("Offline; seat retained", "离线，座位保留")} />{seat.isHost && <Crown size={15} className={styles.hostCrown} />}</button><span className={styles.circleNumber}>{index + 1}</span>{seat.isSheriff && <Medal className={styles.sheriffBadge} size={20} weight="fill" aria-label={t("Sheriff badge", "警徽")} />}</div>
