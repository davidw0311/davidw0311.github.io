...
function Modal({ title, children, onClose, dismissOutside = false }: { title: string; children: ReactNode; onClose: () => void; dismissOutside?: boolean }) {
  ...
  return <dialog ref={ref} className={styles.modal} aria-labelledby={titleId} onCancel={onClose} onClick={event => { if (!dismissOutside || event.target !== event.currentTarget) return; const box = event.currentTarget.getBoundingClientRect(); if (event.clientX < box.left || event.clientX > box.right || event.clientY < box.top || event.clientY > box.bottom) onClose(); }}><div className={styles.modalHeading}><h2 id={titleId}>{title}</h2><button className={styles.iconButton} onClick={onClose} aria-label="Close / 关闭"><X size={22} /></button></div>{children}</dialog>;
}
...
function ActionPanel({ view, lang, disabled: controlsDisabled, send }: { view: GameView; lang: Language; disabled: boolean; send: (command: Command) => Promise<boolean> }) {
  ...
  return <div className={styles.actionPanel}><div className={styles.sectionHeading}><h2>{voting ? <HandPalm size={20} /> : <Eye size={20} />}{voting ? t("Your vote", "你的投票") : action ? t("Your private action", "你的秘密行动") : t("Daytime ability", "白天技能")}</h2>{action?.alreadySubmitted && <span className={styles.submitted}><CheckCircle size={16} />{view.phase.nightStage && action.step !== "wolves" ? t("Submitted · waiting", "已提交 · 等待中") : t("Submitted · can update", "已提交 · 可修改")}</span>}</div>
    {!!view.me?.packVotes?.length && <div className={styles.packVotes}><strong>{t("Pack choices · only wolves can see these", "狼队选择 · 仅狼队可见")}</strong><p>{t("Every wolf must choose. Continue when a majority agrees on a target, or all wolves choose Kill nobody. You can change your choice until then.", "所有狼人必须选择。超过半数同意同一目标，或全体选择空刀后继续。在此之前可以修改选择。")}</p>{view.me.packVotes.map(vote => <p key={vote.seatId}>{view.seats.find(seat => seat.id === vote.seatId)?.name} → {vote.submitted ? vote.targetId ? view.seats.find(seat => seat.id === vote.targetId)?.name : t("Kill nobody", "空刀") : t("Choosing", "选择中")}</p>)}</div>}
    ...
    {action?.step === "guard" && <p className={styles.actionHelp}>{t("You may protect nobody. You cannot protect the same player on consecutive nights.", "可以选择空守，不能连续两夜守护同一名玩家。")}</p>}
