...
export default function WerewolfApp() {
  ...
  return <main className={styles.app} lang={lang === "zh" ? "zh-CN" : "en"}>
    ...
    {view && <div className={styles.roomLayout}>
      ...
      <div className={styles.gameColumns}>
        <section className={styles.tablePanel}>
          ...
          {role && <div className={styles.cardToolbar}><button className={styles.primaryButton} onClick={() => {setRevealed(true);setModal("card");}}><LockKey size={18}/>{t("My card", "我的身份牌")}</button><button className={styles.secondaryButton} onClick={() => setModal("history")}>{t("My actions", "我的行动")}</button>{view.phase.kind === "ready" && <span>{view.seats.filter(seat=>seat.ready).length}/{view.seats.length} {t("ready", "已准备")}</span>}</div>}
    ...
    {modal === "card" && view && <Modal title={t("Your identity", "你的身份")} onClose={() => {setRevealed(false);setModal(null);}}>
            ...
            {view.phase.kind === "ready" && view.me && <div className={styles.readyPanel}><button className={styles.primaryButton} disabled={disabled || (seenKey !== identityKey && !view.me.ready)} onClick={() => void send({ type: "ready", ready: !view.me?.ready })}>{view.me.ready ? t("Ready · undo", "已准备 · 取消准备") : t("I have read my card · ready", "已查看身份 · 准备")}</button><span>{view.seats.filter(seat => seat.ready).length} / {view.seats.length} {t("ready", "人已准备")}</span></div>}
    </Modal>}
