...
import { ArrowLeft, ArrowRight, ArrowClockwise, BookOpen, Check, CheckCircle, Circle, Copy, Crown, Eye, EyeSlash, GearSix, HandPalm, Heart, Hourglass, LockKey, MaskHappy, Moon, MusicNotes, Pause, Play, Plus, QrCode, Shield, SignOut, SpeakerHigh, Sun, Translate, Trash, Users, WarningCircle, WifiHigh, WifiSlash, X } from "@phosphor-icons/react";
...
export default function WerewolfApp() {
  ...
  useEffect(() => {
    ...
    if (!audio) return;
    audio.configure({ voice: audioEnabled, music: musicEnabled, track: musicTrack, language: lang, active: !!view?.me && !startingNight });
    if (view?.phase) audio.updatePhase(view.phase);
  }, [audioEnabled, musicEnabled, musicTrack, lang, view?.me, view?.phase, room.connectivity, startingNight]);
  ...
  return <main className={styles.app} lang={lang === "zh" ? "zh-CN" : "en"}>
    ...
    {view && <StageBanner view={view} lang={lang} />}
      ...
      <div className={styles.gameColumns}>
        ...
        <aside className={styles.sidebar}>
          <section className={styles.identityPanel}><div className={styles.sectionHeading}><h2><LockKey size={16} />{t("Your identity", "你的身份")}</h2>{role && <button className={styles.iconButton} onClick={() => setRevealed(!revealed)} aria-label={revealed ? t("Hide role", "隐藏身份") : t("Reveal role", "查看身份")}>{revealed ? <EyeSlash size={19} /> : <Eye size={19} />}</button>}</div>
            ...
            {view.me && <><button className={styles.textButton} onClick={() => setModal("profile")}>{t("Change profile photo", "更换头像")}</button><button className={styles.secondaryButton} onClick={() => setModal("history")}><BookOpen size={17} />{t("My actions & results", "我的行动与结果")}</button></>}
          </section>
    ...
    {modal === "profile" && view?.me && <Modal title={t("Your profile photo", "你的头像")} onClose={() => setModal(null)}><ProfilePicker lang={lang} disabled={disabled} send={send} onDone={() => setModal(null)} /></Modal>}
...
function ActionPanel({ view, lang, disabled: controlsDisabled, send }: { view: GameView; lang: Language; disabled: boolean; send: (command: Command) => Promise<boolean> }) {
  ...
  return <div className={styles.actionPanel}><div className={styles.sectionHeading}><h2>{voting ? <HandPalm size={20} /> : <Eye size={20} />}{voting ? t("Your vote", "你的投票") : action ? t("Your private action", "你的秘密行动") : t("Daytime ability", "白天技能")}</h2>{action?.alreadySubmitted && <span className={styles.submitted}><CheckCircle size={16} />{view.phase.nightStage && action.step !== "wolves" ? t("Submitted · waiting", "已提交 · 等待中") : t("Submitted · can update", "已提交 · 可修改")}</span>}</div>
    {!!view.me?.packVotes?.length && <div className={styles.packVotes}><strong>{t("Pack choices · only wolves can see these", "狼队选择 · 仅狼队可见")}</strong><p>{t("You can change your choice. Every wolf must choose, and more than half must agree on one target before continuing. Abstentions do not form a majority.", "可以随时修改选择。所有狼人选择后，超过半数同意同一目标才会继续。弃权不构成多数。")}</p>{view.me.packVotes.map(vote => <p key={vote.seatId}>{view.seats.find(seat => seat.id === vote.seatId)?.name} → {vote.submitted ? vote.targetId ? view.seats.find(seat => seat.id === vote.targetId)?.name : t("Abstain", "弃权") : t("Choosing", "选择中")}</p>)}</div>}
