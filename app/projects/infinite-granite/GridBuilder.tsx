...
const abbreviations: Record<ComponentKind,string> = {base:'CAB',upper:'WALL',island:'ISL',sink:'SINK',range:'RNG',fridge:'FRG',dishwasher:'DW',pantry:'PAN'};
export default function GridBuilder({ design, onBuild, onCancel }: {design:KitchenDesign;onBuild:(design:KitchenDesign)=>void;onCancel:()=>void}) {
  ...
  return <section className={styles.gridBuilder} aria-label="Custom kitchen grid builder">
    <header><div><span className={styles.eyebrow}>BUILD FROM A BLANK PLAN</span><h3>Your kitchen, square by square.</h3></div><button aria-label="Close grid builder" onClick={onCancel}><X size={19}/></button></header>
    ...
    <div className={styles.gridScroll}><div className={styles.cellGrid} role="group" aria-label={`${size} by ${size} kitchen grid`} style={{minWidth:size*40,gridTemplateColumns:`repeat(${size}, minmax(0,1fr))`}}>{Array.from({length:size*size},(_,index)=>{
    ...
    <button className={styles.buildGridButton} onClick={build}><Check size={17}/>Build this kitchen<ArrowRight size={17}/></button>
    <button className={styles.textButton} onClick={onCancel}><SquaresFour size={14}/> Back to existing kitchen</button>
  </section>;
}
