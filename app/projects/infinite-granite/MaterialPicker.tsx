...
export default function MaterialPicker({ value, onChange, label='Countertop material' }: { value:string; onChange:(id:string)=>void; label?:string }) {
  ...
  return <div className={styles.catalogue}>
    ...
    <p className={styles.catalogueCount} aria-live="polite">{matches.length} {matches.length===1?'finish':'finishes'}{query&&` matching “${query}”`}</p>
    {current&&<div className={styles.selectedMaterial}><span>SELECTED</span><strong>{current.company} / {current.code}</strong><p>{current.name} · {current.family}</p>{current.textureKind==='detail'&&<p className={styles.supplierNote}>Detail photograph · pattern scale is approximate.</p>}{current.sourceUrl&&<a href={current.sourceUrl} target="_blank" rel="noreferrer">View supplier details ↗</a>}</div>}
    {MATERIAL_COMPANIES.map(name=>{
      const entries=matches.filter(m=>m.company===name);if(!entries.length)return null;
      const all=!!query||!!company||expanded.includes(name);const visible=all?entries:entries.slice(0,12);
      return <section key={name} className={styles.supplierGroup} aria-label={`${name} countertop catalogue`}><div className={styles.supplierHeading}><h4>{name}</h4><span>{entries.length}</span></div>
        ...
        {!all&&entries.length>12&&<button className={styles.showCatalogue} onClick={()=>setExpanded(items=>[...items,name])}>Show all {entries.length} {name} finishes</button>}
      </section>;
    })}
