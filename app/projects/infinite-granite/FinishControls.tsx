...
export default function FinishControls({ tab, design, onChange, onMatchCupboards, onArrangeSink }: FinishControlsProps) {
  return <>
    ...
    {tab === 'cupboards' && <>
      ...
      <ColorPicker label="Wall cupboards" value={design.upperColor} onChange={v => onChange('upperColor', v)} />
      <ColorPicker label="Island" value={design.islandColor} onChange={v => onChange('islandColor', v)} />
      <button className={styles.textButton} onClick={onMatchCupboards}>Match wall cupboards & island to base</button>
      ...
      <FinishPicker label="Handles & faucet" value={design.hardware} onChange={v => onChange('hardware', v)} />
    </>}
    {tab === 'sink' && <>
      ...
      <Field label="Faucet shape"><Choices label="Faucet shape" value={design.faucet} items={[{ id: 'arc', name: 'High arc' }, { id: 'square', name: 'Square neck' }]} onChange={v => onChange('faucet', v)} /></Field>
      <FinishPicker label="Faucet & handles" value={design.hardware} onChange={v => onChange('hardware', v)} />
