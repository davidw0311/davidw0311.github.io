...
import {BASE_IMAGE,WIDTH,HEIGHT,SURFACES,UPPERS,LOWERS,OCCLUDERS,HARDWARE,projection,type Point,type ShowcaseDesign} from './model';
...
export async function createShowcase(canvasElement:HTMLCanvasElement){
 ...
 const source=ctx.getImageData(0,0,WIDTH,HEIGHT),mask=canvas(),mc=mask.getContext('2d',{willReadFrequently:true})!;
 const paint=(points:readonly Point[],id:number)=>{mc.fillStyle=`rgb(${id},0,0)`;mc.beginPath();points.forEach(([x,y],i)=>i?mc.lineTo(x,y):mc.moveTo(x,y));mc.closePath();mc.fill();};
 LOWERS.forEach(p=>paint(p,2));UPPERS.forEach(p=>paint(p,1));SURFACES.forEach((s,i)=>paint(s.quad,i+3));OCCLUDERS.forEach(p=>paint(p,0));
 const masks=mc.getImageData(0,0,WIDTH,HEIGHT).data,uv=new Float32Array(WIDTH*HEIGHT*2),maps=SURFACES.map(s=>projection(s.quad,s.uv));
 for(let p=0;p<WIDTH*HEIGHT;p++){const i=p*4,id=masks[i],x=p%WIDTH,y=Math.floor(p/WIDTH);if(id>0&&id<=2&&source.data[i]>source.data[i+2]*1.35&&source.data[i+1]>source.data[i+2]*1.12&&HARDWARE.some(([l,t,r,b])=>x>=l&&x<=r&&y>=t&&y<=b))masks[i]=0;if(id>=3&&id<SURFACES.length+3){const [u,v]=maps[id-3](p%WIDTH+.5,Math.floor(p/WIDTH)+.5);uv[p*2]=u;uv[p*2+1]=v;}}
 ...
 return {
  async draw(design:ShowcaseDesign){
   ...
   for(let p=0;p<WIDTH*HEIGHT;p++){
    const i=p*4,id=masks[i];if(!id)continue;const a=masks[i+3]/255,r=source.data[i],g=source.data[i+1],b=source.data[i+2],light=r*.2126+g*.7152+b*.0722;
    if(id<=2){
     ...
     for(let k=0;k<3;k++)result.data[i+k]=source.data[i+k]*(1-a)+Math.min(255,color[k]*shade)*a;
    }else if(id<SURFACES.length+3){
     let u=uv[p*2],v=uv[p*2+1];if(design.rotation===90)[u,v]=[v,120-u];
     ...
     for(let k=0;k<3;k++){const at=(xx:number,yy:number)=>t.data[(yy*t.width+xx)*4+k];const color=(at(x0,y0)*(1-fx)+at(Math.min(x0+1,t.width-1),y0)*fx)*(1-fy)+(at(x0,Math.min(y0+1,t.height-1))*(1-fx)+at(Math.min(x0+1,t.width-1),Math.min(y0+1,t.height-1))*fx)*fy;result.data[i+k]=source.data[i+k]*(1-a)+(color*shade+reflection)*a;}
    }
   }
