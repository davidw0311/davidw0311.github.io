...
export default function AccountShell({ children, title, intro }: { children: ReactNode; title: string; intro: string }) {
  return <main className={`${shell.page} ${styles.accountPage}`}>
    <nav className={`${shell.nav} ${styles.accountNav}`} aria-label="Piano Party navigation"><Link href="/projects/piano-party/"><ArrowLeft size={18} /> Piano Party</Link><Link href="/projects/piano-party/practice/">Keep practicing</Link></nav>
