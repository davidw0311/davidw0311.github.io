...
export function QuartzGallery() {
  ...
  return <section className={s.gallery} aria-labelledby="quartz-gallery-title">
    ...
    <QuartzCarousel paused={detailOpen} count={looks.length}>{duplicate => looks.map((look, i) => <article className={s.card} key={look.id} data-carousel-slide>
    ...
    <dialog ref={dialog} className={s.dialog} aria-label={`${selected.supplier} selection`} onClose={() => setDetailOpen(false)} onClick={e => { if (e.target === dialog.current) dialog.current.close(); }} onKeyDown={e => { if (e.key === 'ArrowRight') { e.preventDefault(); move(1); } if (e.key === 'ArrowLeft') { e.preventDefault(); move(-1); } }}>
      ...
      <div className={s.details}><p className={s.eyebrow}>{selected.supplier} · {selected.code}</p><h2>{selected.title}</h2><p>{selected.description}</p>{selected.roomImage && <div className={s.viewToggle} aria-label="Product image view"><button aria-pressed={!roomView} onClick={() => setRoomView(false)}>Slab pattern</button><button aria-pressed={roomView} onClick={() => setRoomView(true)}>In a room</button></div>}<div className={s.tags}><span>{tones.find(item => item.key === selected.tone)?.label}</span><span>{selected.pattern}</span></div><a className={s.enquire} href={lookEmail([selected])}>Enquire about this selection <ArrowUpRight size={20} /></a><button className={s.saveDetail} onClick={() => toggleSave(selected.id)} aria-pressed={saved.includes(selected.id)}><Heart size={20} weight={saved.includes(selected.id) ? 'fill' : 'regular'} />{saved.includes(selected.id) ? 'Saved to your favourites' : 'Save to your favourites'}</button><p className={s.note}>{selected.supplier} catalogue {roomView || selected.imageKind === 'room' ? 'room illustration' : 'slab image'}. Confirm colour, availability and pricing with our team.</p><a className={s.source} href={selected.source} target="_blank" rel="noopener noreferrer">View supplier details <ArrowUpRight size={17} /></a></div>
    </dialog>
  </section>;
}
