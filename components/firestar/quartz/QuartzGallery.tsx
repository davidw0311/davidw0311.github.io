...
export function QuartzGallery() {
  ...
  const openLook = (look: Look) => { setViewerLooks(filtered); setSelected(look); setRoomView(false); dialog.current?.showModal(); };
  ...
  return <section className={s.gallery} aria-labelledby="quartz-gallery-title">
    ...
    <div className={s.galleryMeta}><p role="status">{filtered.length} {filtered.length === 1 ? 'selection' : 'selections'} to explore{savedOnly ? ' · Your favourites' : ''}</p><span>Explore a surface. Find your favourite.</span></div>
    ...
    <dialog ref={dialog} className={s.dialog} aria-label="Kasa Quartz selection" onClick={e => { if (e.target === dialog.current) dialog.current.close(); }} onKeyDown={e => { if (e.key === 'ArrowRight') { e.preventDefault(); move(1); } if (e.key === 'ArrowLeft') { e.preventDefault(); move(-1); } }}>
