...
export function QuartzGallery() {
  ...
  return <section className={s.gallery} aria-labelledby="quartz-gallery-title">
    ...
    <QuartzCarousel paused={detailOpen} count={looks.length}>{duplicate => looks.map((look, i) => <article className={s.card} key={look.id} data-carousel-slide>
      ...
      <button className={s.cardTitle} tabIndex={duplicate ? -1 : undefined} onMouseDown={duplicate ? event => event.preventDefault() : undefined} onClick={() => openLook(look)} aria-label={`View details for ${look.title}`}><div><h2>{look.title}</h2><p>{look.code}</p></div><ArrowUpRight size={23} /></button>
